...
import re
from .models import Projects,Remarks
...
def homepage(request):
    ...
    if sort_by:
        proList = Projects.objects.order_by(sort_by)
    else:
        proList = Projects.objects.all()
    ...
    return render(request,'project/HomeP.html',params)
...
def Edit(request,id):
    if request.method == "POST":
        ...
        # validation over
        try:
            Assignments=Projects.objects.get(id=id)
            Assignments.updated_on = datetime.now()
            Assignments.email=email
            Assignments.title=title
            Assignments.desc=desc
            Assignments.enroll_no=enroll_no
            Assignments.year=year
            Assignments.sec=sec
            Assignments.git_link=git_link
            Assignments.live_link= live_link if (len(str(live_link))>0) else None
            Assignments.screenshot= screenshot if (not (request.FILES==None))else None
            Assignments.submitted_by=submitted_by
            Assignments.save()
            messages.add_message(request, messages.SUCCESS,"Project Edited successfull")
            return redirect("/project/")
        except Exception as e:
                messages.add_message(request, messages.ERROR, e)
                return redirect("/project/")
...
def Search(request):
    ...
    proList = Projects.objects.filter(title__contains=searched)
    ...
    return render(request,"project/searchkaro.html",params)
